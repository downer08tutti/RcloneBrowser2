...
void MainWindow::addStream(const QString &remote, const QString &stream) {
  ...
  QStringList streamArgs = QProcess::splitCommand(stream);
  QString streamProgram = streamArgs.takeFirst();
  player->start(streamProgram, streamArgs, QProcess::ReadOnly);
